...
- (void)setAutoAdjustTrackColor:(BOOL)autoAdjust {
  ...
  // setMinimumTrackTintColor has been overridden to also set
  // autoAdjustTrackColor to NO
  // therefore super's implementation must be called to set
...
// return the currently displayed color if possible, otherwise return
// _popUpViewColor
// if animated colors are set, the color will change each time the slider value
// changes
...
- (void)setPopUpViewAnimatedColors:(NSArray *)colors
                     withPositions:(NSArray *)positions {
  if (positions) {
    NSAssert([colors count] == [positions count],
             @"popUpViewAnimatedColors and locations should contain the same "
             @"number of items");
  }
...
// when either the min/max value or number formatter changes, recalculate the
// popUpView width
...
- (NSNumberFormatter *)numberFormatter {
  return [_numberFormatter copy]; // return a copy to prevent formatter
                                  // properties changing and causing mayhem
}
...
- (CGRect)trackRectForBounds:(CGRect)bounds {
  if (self.barHeight == 0) {
      return CGRectMake(bounds.origin.x, bounds.origin.y, bounds.size.width,
                        1);
  } else {
    return CGRectMake(bounds.origin.x, bounds.origin.y, bounds.size.width,
                      self.barHeight);
...
- (void)setup {
  ...
  self.popUpViewColor =
      [UIColor colorWithHue:0.6 saturation:0.6 brightness:0.5 alpha:0.8];
...
- (void)didBecomeActiveNotification:(NSNotification *)note {
  if (self.popUpViewAnimatedColors) {
    [self.popUpView setAnimatedColors:_popUpViewAnimatedColors
                         withKeyTimes:_keyTimes];
  }
}

- (void)updatePopUpView {
  NSString *valueString; // ask dataSource for string, if nil or blank, get
                         // string from _numberFormatter
  ...
  CGRect popUpRect = CGRectInset(thumbRect, (thumbW - popUpViewSize.width) / 2,
                                 (thumbH - popUpViewSize.height) / 2);
  ...
  CGFloat offset =
      minOffsetX < 0.0 ? minOffsetX : (maxOffsetX > 0.0 ? maxOffsetX : 0.0);
...
- (CGSize)calculatePopUpViewSize {
  ...
  CGSize minValSize = [self.popUpView
      popUpSizeForString:[_numberFormatter
                             stringFromNumber:@(self.minimumValue)]];
  CGSize maxValSize = [self.popUpView
      popUpSizeForString:[_numberFormatter
                             stringFromNumber:@(self.maximumValue)]];

  return (minValSize.width >= maxValSize.width) ? minValSize : maxValSize;
}

// takes an array of NSNumbers in the range self.minimumValue -
// self.maximumValue
...
- (void)_hidePopUpViewAnimated:(BOOL)animated {
  ...
  [self.popUpView
      ...
      completionBlock:^{
        if ([self.delegate
                respondsToSelector:@selector(sliderDidHidePopUpView:)]) {
...
- (void)setValue:(float)value {
  ...
  [self.popUpView setAnimationOffset:[self currentValueOffset]
                         returnColor:^(UIColor *opaqueReturnColor) {
    ...
    [self.popUpView animateBlock:^(CFTimeInterval duration) {
      [UIView animateWithDuration:duration
                       animations:^{
                         ...
                         [self.popUpView
                             setAnimationOffset:[self currentValueOffset]
                                    returnColor:^(UIColor *opaqueReturnColor) {
...
- (void)setMinimumTrackTintColor:(UIColor *)color {
  self.autoAdjustTrackColor =
      NO; // if a custom value is set then prevent auto coloring
...
- (void)cancelTrackingWithEvent:(UIEvent *)event {
  ...
  if (self.popUpViewAlwaysOn == NO)
    [self _hidePopUpViewAnimated:YES];
}
